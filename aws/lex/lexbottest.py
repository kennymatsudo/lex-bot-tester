        ...
        * the slots contain the specified values, as declared in the item
        ...
        for c in conversations:
            ...
            for ci in c:
                ...
                session_attributes = self.csc.get_session_attributes()
                if verbose:
                    if before_message:
                        print(Color.colorize(' Bot: {}'.format(
                            before_message), Color.WHITE, Color.BRIGHT_BLUE))
                ...
                if use_tts:
                    response = self.csc.post_text_to_speech(
                        ci.send, session_attributes=session_attributes)
                else:
                    response = self.csc.post_text(
                        ci.send, session_attributes=session_attributes)
                self.assertEqual(expected_result.intent_name,
                                 self.csc.get_intent_name())
                ...
                if len(expected_result) > 0:
                    for rk in expected_result.keys():
                        try:
                            ...
                            print('ERROR: rk={} msg={}'.format(
                                rk, str(ex)), file=sys.stderr)
                            if before_dialog_state == DialogState.ELICIT_SLOT and slot_to_elicit is not None:
                                self.assertEqual(ci.send.lower(), self.csc.get_slot(
                                    slot_to_elicit).lower())
    ...
    def conversations_text_helper(self, bot_alias, bot_name, user_id, conversation_definition, verbose=VERBOSE,
        ...
        for i in lmc.get_intents_for_bot():
                ...
                for cdi in conversation_definition[i]:
                    if len(cdi) != 3:
                        raise AttributeError(
                            'Expected item with len=3 (actual len={})'.format(len(cdi)))
                    kwargs = {}
                    for k in cdi[2]:
            ...
            self.conversations_text(
                bot_name, bot_alias, user_id, conversations, verbose, use_tts)
